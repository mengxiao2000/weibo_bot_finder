...
with tab2:
    st.write('该工具利用微博可公开获取的社交账号信息作为实验数据集。从新浪微博社交机器人商家处购买了不同智能水平的机器人账号共247个，再从亲友处、非广告的微博评论中以及具有代表性的公众人物、机构账号中人工筛选出人类账号共255个，利用爬虫采集这两类账号的数据，构建了账号原始数据集。然后，将获得的502个新浪微博账号数据进行数据预处理。参考以往文献在社交机器人识别方面使用的账号级别、内容级别等方面的特征，本文对每一个账号共抽取了25个特征，包括用户的性别、等级、是否认证、关注数、粉丝数、关注数和粉丝数之比、发布内容的原创率、发布频率等。选择随机森林、XGboost、决策树、支持向量机、Logistic回归、感知机、KNN和朴素贝叶斯作为二分类机器学习算法，构建社交机器人检测模型。为比较模型准确度，采用5折交叉验证，XGBoost模型是在测试集上预测性能最优的模型，它的准确率为96.02%，F1值为95.89，在各指标上的预测结果全面超越其他机器学习模型。因此，基于XGboost模型进行社交机器人识别。')
with tab3:
